...
public class EchoServerHandler extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
        //Common.writeAndFlush(ctx, true, "Server receive", true, (ByteBuf) msg);
        //super.channelRead(ctx, msg);
        ByteBuf in = (ByteBuf) msg;
        //将消息记录到控制台
        String strForTime = formatter.format(ZonedDateTime.now());
        String message = String.format("[%s][Server receive]:%s",
                strForTime,
                in.toString(CharsetUtil.UTF_8));
        System.out.println("[" + ctx.channel().remoteAddress() + "]" + message);
        //将接收到的消息写给发送者,而不冲刷出站消息

        ctx.write(Unpooled.copiedBuffer(message.getBytes(Charset.forName("UTF-8"))));
    }
